...
UserBasedCF <- function(train_data, test_data, N, NN, onlyNew=TRUE){
  ...
  res <- list(prediction, TopN, similarity_matrix)
  names(res) <- c('prediction', 'topN', 'similarity_matrix')
...
UserBasedCFOneUser <- function(dataset, user, N, NN, onlyNew=TRUE){
  ...
  for (i in rownames(dataset)){
    if (i != user){
      sim <- sum(((dataset[user, ])-(mean(dataset[user, ],na.rm=T)))
                 *((dataset[i,])-(mean(dataset[i, ],na.rm=T))), na.rm=TRUE)/
        sqrt((sum((dataset[user,]) - (mean(dataset[user, ],na.rm=T)), na.rm = T)^2) *
               (sum((dataset[i, ])-(mean(dataset[i, ],na.rm=T)), na.rm = T)^2))
  ...
  ### Nearest Neighbors ###
  similarity_NN_vec <- similarity_vect
  similarity_NN_vec[is.infinite(similarity_NN_vec)] <- NA
  similarity_NN_vec[is.nan(similarity_NN_vec)] <- NA
  crit_val <- -sort(-similarity_NN_vec)[NN]
  similarity_vect <- na.omit(ifelse(similarity_NN_vec >= crit_val, similarity_NN_vec, NA))
  ...
  TopN_Score <- head(-sort(-prediction), N)
  TopN_Artists <- names(TopN_Score)

  res <- data.frame(TopN_Artists,TopN_Score, row.names = 1:length(TopN_Artists))
